...
& python -B (Join-Path $root 'tools/garmin-firmware/emulate_neural_specimen_n64_controls.py') --check-evidence
...
$sources=@((Join-Path $target 'hook.S'),(Join-Path $target 'overlay.c'),
 (Join-Path $target 'renderer.c'),(Join-Path $target 'brain64_packed.c'))
$objects=@()
...
$elf=Join-Path $build 'fr245-1370-neural-specimen-n64-controls.elf';$map=Join-Path $build 'fr245-1370-neural-specimen-n64-controls.map'
...
& $objcopy --dump-section ".hook=$(Join-Path $build 'hook.bin')" --dump-section ".keyhook=$(Join-Path $build 'keyhook.bin')" --dump-section ".primary=$(Join-Path $build 'primary.bin')" --dump-section ".secondary=$(Join-Path $build 'secondary.bin')" $elf
if($LASTEXITCODE){throw 'extract failed'}
((& $objdump -d -s $elf) -join "`n").Replace($build,'<BUILD>') + "`n" | Set-Content (Join-Path $build 'fr245-1370-neural-specimen-n64-controls.disassembly.txt') -Encoding ascii -NoNewline
& $nm -n $elf | Set-Content (Join-Path $build 'symbols.txt') -Encoding ascii
if(@(& $nm -u $elf).Count){throw 'undefined symbols'}
& python -B (Join-Path $root 'tools/garmin-firmware/emulate_neural_specimen_n64_controls.py') --check-build $build
if($LASTEXITCODE){throw 'offline N64 build gate failed'}
if($EvidenceRoot){
 & python -B (Join-Path $root 'tools/garmin-firmware/emulate_neural_specimen_n64_controls.py') --build $build --publish-evidence $EvidenceRoot
