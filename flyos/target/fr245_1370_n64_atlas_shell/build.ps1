...
& python -B (Join-Path $root 'tools/garmin-firmware/emulate_n64_atlas_shell.py') --check-evidence
...
$elf=Join-Path $build 'fr245-1370-n64-atlas-shell.elf';$map=Join-Path $build 'fr245-1370-n64-atlas-shell.map'
...
((& $objdump -d -s $elf) -join "`n").Replace($build,'<BUILD>') + "`n" | Set-Content (Join-Path $build 'fr245-1370-n64-atlas-shell.disassembly.txt') -Encoding ascii -NoNewline
& $nm -n $elf | Set-Content (Join-Path $build 'symbols.txt') -Encoding ascii
if(@(& $nm -u $elf).Count){throw 'undefined symbols'}
& python -B (Join-Path $root 'tools/garmin-firmware/emulate_n64_atlas_shell.py') --check-build $build
if($LASTEXITCODE){throw 'offline N64 build gate failed'}
if($EvidenceRoot){
 & python -B (Join-Path $root 'tools/garmin-firmware/emulate_n64_atlas_shell.py') --build $build --publish-evidence $EvidenceRoot
