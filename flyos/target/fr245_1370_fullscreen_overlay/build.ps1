...
$elf = Join-Path $buildRoot 'fr245-1370-fullscreen-overlay.elf'
$map = Join-Path $buildRoot 'fr245-1370-fullscreen-overlay.map'
$hook = Join-Path $buildRoot 'hook.bin'
$payload = Join-Path $buildRoot 'overlay.bin'
$listing = Join-Path $buildRoot 'fr245-1370-fullscreen-overlay.disassembly.txt'
...
if ($LASTEXITCODE -ne 0) { throw 'Full-screen overlay link failed' }
