#!/usr/bin/env python3
"""Build and exactly verify the offline-only FR245 N64-controls package pair.

The tool emits only synthetic 13.76/13.77 analysis artifacts beneath the
fixed local quarantine.  It has no device discovery, staging, reset, eject, or
Garmin-software integration path.
"""
...
CONTROLS_DECISION_RELATIVE_PATH = Path(
    "artifacts/analysis/fr245-1370-n64-controls-offline-construction-decision.json"
)
TARGET_BUILD_RELATIVE_PATH = Path(
    "flyos/target/fr245_1370_neural_specimen_n64_controls/build"
)

# Filled only after the controls build and emulator evidence are final.  An
# unpinned build is deliberately impossible to package.
TARGET_MANIFEST_SHA256 = "28243fab77447f1ce12ad05680878868afe4482dd7465ee3a12c17ae2e6322f8"
CONTROLS_DECISION_SHA256 = "15bc970f3277ffb61f2562ac52fc5f779f9fe7e47c116f7f79bf4fba025d1f54"

REQUIRED_SUFFIX = ".gcd.analysis-only.DO_NOT_INSTALL"
CANDIDATE_FILENAME = (
    "Forerunner245_1376-flyos-neural-specimen-n64-controls"
    ".gcd.analysis-only.DO_NOT_INSTALL"
)
RESTORE_FILENAME = (
    "Forerunner245_1377-official-payload-restore-n64-controls"
    ".gcd.analysis-only.DO_NOT_INSTALL"
)
BUILD_REPORT_FILENAME = "neural-specimen-n64-controls-package-build-1376-1377.json"
STRICT_REPORT_FILENAME = (
    "neural-specimen-n64-controls-package-strict-verification-1376-1377.json"
)
SHA_LEDGER_FILENAME = "neural-specimen-n64-controls-package-SHA256SUMS.txt"

MAIN_RECORD_ID = 0x02BD
HELPER_RECORD_ID = 0x0505
EXPECTED_HWID = 3076
OFFICIAL_VERSION = 1370
PRIOR_CANDIDATE_VERSION = 1374
PRIOR_RESTORE_VERSION = 1375
CANDIDATE_VERSION = 1376
RESTORE_VERSION = 1377
...
def _validate_manifest(manifest: dict[str, Any]) -> None:
    if manifest.get("schema") != "flyos.fr245.n64-controls-target.v1":
        raise ValueError("controls target manifest schema changed")
    if manifest.get("link_and_emulate_allowed") is not True:
        raise ValueError("controls target is not approved for offline link/emulation")
    if manifest.get("packaging_allowed") is not False:
        raise ValueError("controls target must remain blocked from live packaging")
    if manifest.get("repair_byte") != PRIMARY_REPAIR_VA:
        raise ValueError("controls target repair byte changed")
    segments = manifest.get("segments")
    if not isinstance(segments, dict) or set(segments) != set(SEGMENT_LAYOUT):
        raise ValueError("controls target segment inventory changed")
    files = manifest.get("files")
    if not isinstance(files, dict):
        raise ValueError("controls target file inventory is absent")
    for name, (start, minimum, maximum) in SEGMENT_LAYOUT.items():
        item = segments[name]
        if set(item) != {"start", "end", "size", "sha256"}:
            raise ValueError(f"controls target {name} fields changed")
        if (
            item["start"] != start
            ...
            raise ValueError(f"controls target {name} placement changed")
    symbols = manifest.get("symbols")
    if not isinstance(symbols, dict):
        raise ValueError("controls target symbol inventory is absent")
    if symbols.get("n64_overlay_then_flush", {}).get("address") != PRIMARY_VA:
        raise ValueError("controls display entry address changed")
    key_target = symbols.get("flyos_key_event", {}).get("address")
    if not isinstance(key_target, int):
        raise ValueError("controls key entry address is absent")
    if not (
        PRIMARY_VA <= key_target < PRIMARY_VA + PRIMARY_ALLOCATION
        or SECONDARY_VA <= key_target < SECONDARY_VA + SECONDARY_ALLOCATION
    ):
        raise ValueError("controls key entry is outside the audited allocations")
...
def _validate_controls_decision(decision: dict[str, Any]) -> None:
    _require_exact_keys(
        decision,
        {
            "schema_version",
    ...
    if (
        ...
        raise ValueError("controls decision policy changed")
    if decision["device"] != {
        ...
        raise ValueError("controls decision version chain changed")
    if decision["containment"] != {
        ...
        raise ValueError("controls decision containment changed")
    if set(decision["target"]) != {"manifest", "hook", "keyhook", "primary", "secondary"}:
        raise ValueError("controls decision target inventory changed")
    if set(decision["verification"]) != {
        "emulator_report",
        "evidence_manifest",
        "visual_audit",
    }:
        raise ValueError("controls decision verification inventory changed")
    if set(decision["static_evidence"]) != {
        "button_decompile",
        "button_view_decompile",
        "key_queue_decompile",
    }:
        raise ValueError("controls decision static evidence inventory changed")
    ...
    if (
        ...
        raise ValueError("controls decision prior-decision pin changed")
    if decision["target"]["manifest"].get("sha256") != TARGET_MANIFEST_SHA256:
        raise ValueError("controls decision manifest pin changed")
    restrictions = decision["restrictions"]
    if not isinstance(restrictions, list) or not restrictions:
        raise ValueError("controls decision restrictions are absent")
    joined = " ".join(str(item).lower() for item in restrictions)
    for phrase in ("do not stage", "do not install", "no nonboot recovery"):
        if phrase not in joined:
            raise ValueError(f"controls decision restriction is absent: {phrase}")
...
def load_inputs(
    ...
    decision_snapshot = base_strategy.load_pinned_snapshot(
        repo_root / CONTROLS_DECISION_RELATIVE_PATH,
        expected_sha256=CONTROLS_DECISION_SHA256,
    )
    decision = json.loads(decision_snapshot.data.decode("utf-8"))
    _validate_controls_decision(decision)
    ...
    validated = base_strategy.load_and_validate_decision(
        repo_root, repo_root / BASE_DECISION_RELATIVE_PATH
    )
    if validated["decision"]["packaging_allowed"] is not False:
        raise ValueError("base decision no longer blocks live packaging")
    official = validated["snapshots"]["inputs.official_gcd"].data
    build = repo_root / TARGET_BUILD_RELATIVE_PATH
    manifest_snapshot = _decision_snapshot(
        repo_root, decision["target"]["manifest"], "controls manifest"
    )
    manifest = json.loads(manifest_snapshot.data.decode("utf-8"))
    _validate_manifest(manifest)
    segments: dict[str, bytes] = {}
    validated_files = dict(validated["validated_files"])
    validated_files["controls.decision"] = decision_snapshot.metadata()
    validated_files["controls.prior_decision"] = pinned_prior.metadata()
    validated_files["controls.manifest"] = manifest_snapshot.metadata()
    for name in SEGMENT_LAYOUT:
        snapshot = _decision_snapshot(
            repo_root, decision["target"][name], f"controls {name}"
        )
        item = manifest["segments"][name]
        if snapshot.path.resolve() != (build / f"{name}.bin").resolve():
            raise ValueError(f"controls {name} path changed")
        if snapshot.sha256 != item["sha256"] or snapshot.size != item["size"]:
            raise ValueError(f"controls {name} decision and manifest disagree")
        segments[name] = snapshot.data
        validated_files[f"controls.{name}"] = snapshot.metadata()
    for group in ("verification", "static_evidence"):
        for name, item in decision[group].items():
            snapshot = _decision_snapshot(
                repo_root, item, f"controls {group}.{name}"
            )
            validated_files[f"controls.{group}.{name}"] = snapshot.metadata()
    hook_target = build_overlay_candidate.decode_thumb_bl(
        DISPLAY_HOOK_VA, segments["hook"]
    )
    if hook_target != PRIMARY_VA:
        raise ValueError("controls display hook does not branch to primary")
    keyhook = segments["keyhook"]
    if keyhook[4:] != b"\x00\xbf":
        raise ValueError("controls key hook does not end in a Thumb NOP")
    if _decode_thumb_bw(KEY_HOOK_VA, keyhook[:4]) != manifest["symbols"]["flyos_key_event"]["address"]:
        raise ValueError("controls key hook target does not match the manifest")
    return ControlsInputs(
        official=official,
        manifest=manifest,
...
def _construct_candidate(inputs: ControlsInputs) -> tuple[bytes, dict[str, Any]]:
...
def _artifact_report(
    ...
    return {
        ...
        "purpose": (
            "offline-only synthetic 13.76 N64 controls overlay"
            if is_candidate
            else "offline-only synthetic 13.77 wrapper carrying official 13.70 application and resources"
        ),
        "size": len(output),
...
def construct_pair(repo_root: Path) -> tuple[bytes, bytes, dict[str, Any]]:
    ...
    report = {
        "schema_version": 1,
        "tool": "tools/garmin-firmware/neural_specimen_n64_controls_version_strategy.py",
