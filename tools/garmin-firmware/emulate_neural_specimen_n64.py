...
ELF_NAME = "fr245-1370-neural-specimen-n64.elf"
HOOK, PRIMARY, SECONDARY = 0x9A20, 0x1F6000, 0x1FA400
...
def stack_audit(build: Path, symbols: dict) -> dict:
    ...
    external, cross = [], []
    current = None
    listing = (build / "fr245-1370-neural-specimen-n64.disassembly.txt").read_text()
    ...
    require(len(external) == 2, "expected dirty and dispatch transfers only")
    require(sorted(item["function"] for item in external) ==
            ["n64_overlay_then_flush", "n64_overlay_then_flush"],
...
def check_build(build: Path = DEFAULT_BUILD) -> dict:
    ...
    files = [build / ELF_NAME, build / "fr245-1370-neural-specimen-n64.map",
             build / "fr245-1370-neural-specimen-n64.disassembly.txt", build / "symbols.txt",
    ...
    sources = [TARGET / name for name in ("hook.S", "overlay.c", "renderer.c", "linker.ld", "build.ps1")]
    ...
    manifest = {"schema": "flyos.fr245.n64-target.v1", **evidence,
...
def load_build(build: Path = DEFAULT_BUILD) -> Bundle:
    ...
    assert_build_clean(build, manifest)
    bundle = Bundle(build, *((build / name).read_bytes() for name in ("primary.bin", "secondary.bin", "hook.bin")), manifest)
...
def expected_cell(level: int, x: int, y: int) -> set[tuple[int, int]]:
    if level == 0:
        return {(x + 2, y + 2)}
    if level == 1:
        return {(x + 2, y + n) for n in range(5)} | {(x + n, y + 2) for n in range(5)}
    if level == 2:
        return ({(x + n, y) for n in range(5)} | {(x + n, y + 4) for n in range(5)} |
                {(x, y + n) for n in range(5)} | {(x + 4, y + n) for n in range(5)})
    return {(x + a, y + b) for a in range(5) for b in range(5)}
...
def verify_cells(framebuffer: bytes, activation: list[int]) -> int:
    ...
    for neuron, value in enumerate(activation):
        x, y = 96 + (neuron & 7) * 6, 79 + (neuron >> 3) * 8
        region = {(x + a, y + b) for a in range(5) for b in range(5)}
        ...
        role = 0x33 if value < 0 else 0x38 if neuron >= 56 and level == 3 else 0x0C
        for point in region:
...
def emulate(bundle: Bundle, *, home: str = "valid", pressed_mask: int = 0,
            ...
            forced_activation: tuple[int, int] | None = None) -> dict[str, Any]:
    ...
    machine.mem_write(HOOK, bundle.hook); machine.mem_write(PRIMARY, bundle.primary); machine.mem_write(SECONDARY, bundle.secondary)
    ...
    def on_read(uc: Uc, _access: int, address: int, size: int, _value: int, _user: Any) -> None:
        ...
        allowed_fixed = {VIEW_ROOT, GPIOD, GPIOA, GPIOC, BATTERY, USB_MS, RTC_SECONDS, RTC_PRESCALER}
    ...
    oracle = host_oracle(bundle.build, captured.get("tick", 0), pressed_mask, battery_valid,
    ...
    result = {"eligible": eligible, "home": home, "pressed_mask": pressed_mask,
              ...
              "safe_radius": all((x - 120) ** 2 + (y - 120) ** 2 <= 114 ** 2 for x, y in foreground),
...
def generate_report(build: Path, report_path: Path, preview_path: Path | None) -> dict:
    ...
    report = {"schema": "flyos.fr245.n64-emulation.v2", "cases": cases,
...
def publish_evidence(build: Path, output_dir: Path) -> dict:
    ...
    bundle = load_build(build)
    report_path = output_dir / "fr245-1370-n64-emulator-report.json"
    ...
    for stem, frame in fixtures.items():
        ppm = output_dir / f"fr245-1370-n64-{stem}.ppm"
        png = output_dir / f"fr245-1370-n64-{stem}.png"
        write_ppm(frame, ppm); write_png(frame, png); paths += [ppm, png]
    build_paths = [build / "manifest.json", build / "SHA256SUMS.txt",
                   build / "hook.bin", build / "primary.bin", build / "secondary.bin"]
    manifest = {"schema": "flyos.fr245.n64-evidence.v1",
    ...
    manifest_path = output_dir / "fr245-1370-n64-evidence-manifest.json"
